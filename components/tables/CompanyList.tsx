...
// Components
import {AddFilingModal} from 'components/modals';
// next link
        ...
        <TableCell align="left">
          <Box display="flex">
            ...
            <span style={{fontSize: '12px', display: 'flex', alignItems: 'center'}}>
              <Link href={`/company/${row.companyid}/timeseriesdata/`}>
                <a style={{textDecoration: 'none', color: '#000', fontWeight: 'normal'}}>
                ...
                <TableBody>
                  {row.document.map((detailRow: any, index: any) => (
                    <TableRow key={index}>
                      ...
                      <TableCell component="th" scope="row">
                        <Link
                          href={`/company/${row.companyid}/normaliseddata/${detailRow.documentid}/`}
                        >
                          <a
                            className="tooltip"
                      ...
                      >
                        <Typography align="right" className="viewAll">
                          <Link href={'company/' + row.companyid}>
                            <a style={{textDecoration: 'none', color: '#89AFF0'}}>
                              View all&nbsp;&nbsp;
        ...
        <Table size="small" aria-label="collapsible table">
          ...
          <TableBody className="mainpagetablebody">
            ...
            {stableSort(data.results, getComparator(order, orderBy)).map((row, index) => {
              ...
              return <TableData key={index} row={row} handleFilingModel={handleFilingModel} />;
            })}
            {emptyRows > 0 && (
