...
// endpoints
import {COMPANIES_DETAIL, TIMESERIES_PARSED} from 'lib/constants/endpoints';
// services
...
import {MoreHoriz, FormatTextdirectionRToL, FormatTextdirectionLToR} from '@material-ui/icons';
...
const CompanyTimeseries: FC<ICompanyTimeseriesProps> = (props: ICompanyTimeseriesProps) => {
  ...
  storageService.setItem('companyId', company_id);
  ...
  React.useEffect(() => {
    const UpdateState: any = (results) => setExcelData(results.data);
    readRemoteFile('https://libero-notes.s3.ap-south-1.amazonaws.com/factstream/factstream.csv', {
      header: true,
  ...
  if (!company_detail || !table_parsed || !excelData) {
    return (
      <>
