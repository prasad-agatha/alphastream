...
// endpoints
import {COMPANIES_DETAIL, NORMALISED_VALUES, NORMALISED_PERIODS} from 'lib/constants/endpoints';
// Export as excel
import {CSVLink} from 'react-csv';
// components
import {TemplateSelector} from 'components/lookup';
// Data Table
import {CompanyNormalised} from 'components/tables';
// Material ui
...
const NormalisedData: FC = () => {
  ...
  const extractExcelData = () => {
    ...
    extract_data.map((row) => {
      // const newData = {};
      const obj = {};

      obj['displaytablename'] = row.displaytablename;
      obj['normrowitemname'] = row.normrowitemname;
      extract_periods.map((item) => {
        obj[item] = (row[item] && row[item].split('|')[0]) || '';
      });
        ...
        <div>
          <Box display="flex" justifyContent="center" alignItems="center" className="headerTop">
            ...
            <Box mr={3} ml="auto" display="flex" className="tabBox">
              <Box display="flex">
                ...
                <Box mr={2}>
                  <Link href={`/company/${company_detail.companyid}/normalisedpdf/${document_id}`}>
                    <a
                      target="_blank"
                      className="iconColor"
                      style={{
            ...
            <Box mr={1}>
              {company_detail ? (
                <>
                  <Link href={`/company/${company_detail.companyid}/timeseriesdata/`}>
                    <a
          ...
          {/* <Box mt={3} px={3} display="flex" justifyContent="center">
            <Grid container>
                ...
                <Box justifyContent="center" alignItems="center">
                  <Typography>
                    <span className="value-text">
                      {company_detail ? (
                        <>
                          All values are in {company_detail.currency.currencyname},&nbsp;
                          {company_detail.quantum.quantumname}
                        </>
                      ) : (
                        ''
                      )}
              ...
              <Grid item xs={12} className="content-align-justify-end">
