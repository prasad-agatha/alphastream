...
//endpoints
import {COMPANIES_DETAIL, TIMESERIES_VALUES, TIMESERIES_PERIODS} from 'lib/constants/endpoints';
// Export as excel
import {CSVLink} from 'react-csv';
// components
import {TemplateSelector} from 'components/lookup';
// Data Table
import {CompanyTimeseries} from 'components/tables';
// Material ui
...
const TimeseriesData: FC = () => {
  ...
  if (company_id && company_id === '15856') {
    Router.push(`/${company_id}/timeseriesdata`);
  }
    ...
    onSubmit: (values: any) => {
      ...
      if (values.columnOptions.length > 0) {
        if (!data['column'] || !data['years']) {
          ...
          // });
          // setHeadCells(newArr);
          // setExcelData(extractExcelData(newArr));
          getExcelFileName();
        } else {
          ...
          <Box className="content-align-justify-center headerTop">
            ...
            <Box mr={3} ml="auto" display="flex" className="tabBox">
              <Box display="flex">
                ...
                <Box mr={2}>
                  <Link href={`/company/${company_detail.companyid}/timeseriespdf`}>
                    <a
                      target="_blank"
                      className="iconColor"
                      style={{
                ...
                <Box pt={1} style={{borderRight: '1px solid #89aff0', width: '43px'}}>
                  {excelData.length > 0 ? (
                    <CSVLink
                      data={excelData}
                      filename={company_detail.name + (excelName ? excelName : '') + '.csv'}
                      style={{
                        padding: '4px 4px',
                      }}
                    >
